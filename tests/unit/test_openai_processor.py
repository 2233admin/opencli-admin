"""Unit tests for the OpenAI AI processor (AUDIT C8/C25: explicit per-request
timeout + bounded concurrency instead of a sequential await-in-a-for-loop).

The SDK client is mocked at the ``openai.AsyncOpenAI`` class level, same
convention as tests/unit/llm/test_adapters.py — no real network call.
"""
...
async def test_process_sets_explicit_timeout_from_settings_default():
    ...
    mock_client.chat.completions.create = AsyncMock(return_value=_chat_response('{"a": 1}'))
...
async def test_process_timeout_overridable_via_config():
    ...
    with patch("openai.AsyncOpenAI", return_value=mock_client):
        await OpenAIProcessor().process(
            records=[_record("r1")],
            prompt_template="{{content}}",
            config={"base_url": None, "timeout": 45},
        )
...
async def test_process_preserves_record_order_despite_uneven_latency():
    ...
    # record 0's call takes longer than 1/2's, so completion order is 1, 2, 0
    # — but the result list must stay in submission order.
    delays = {"r0": 0.03, "r1": 0.0, "r2": 0.0}

    async def fake_create(**kwargs):
        rec_id = kwargs["messages"][0]["content"]  # prompt renders straight to id
        await asyncio.sleep(delays[rec_id])
        return _chat_response(f'{{"seen": "{rec_id}"}}')
...
@pytest.mark.asyncio
async def test_process_no_openai_package_returns_failed_result():
    """The pre-existing import-availability guard must still short-circuit
    cleanly (not raise) when the openai package isn't installed."""
