...
async def _execute_native_node(
    ...
    *,
    ...
    if binding_id == DEDUPE_BINDING_ID:
        binding = _read_dict(node.runtime.get("binding"))
        binding_input = _read_dict(binding.get("input"))
        key = _read_string(binding_input.get("key")) or "contentHash"
        window = _read_string(binding_input.get("window")) or "24h"
        deduplicated, rejected_count = _dedupe_runtime_items(
            node,
            input_items,
            run_id,
            key=key,
        ...
        return (
            {
                ...
                "metrics": {
                    ...
                    "accepted": len(deduplicated),
                    "duplicates": rejected_count,
                },
                "lineage": _lineage_pointer(node),
            },
            deduplicated,
...
def _dedupe_runtime_items(
    ...
    *,
    ...
    fields = [field.strip() for field in key.split("+") if field.strip()]
    seen: set[str] = set()
    deduplicated: list[dict[str, Any]] = []
    for item in input_items:
        identity = _dedupe_identity(item, fields)
        if identity in seen:
            continue
        seen.add(identity)
        deduplicated.append(_append_lineage(item, node, step="dedupe", run_id=run_id))
    return deduplicated, len(input_items) - len(deduplicated)


def _dedupe_identity(item: dict[str, Any], fields: list[str]) -> str:
    ...
    for field in fields:
        ...
        if value is None:
            value = raw.get(field_name, item.get(field_name))
        values.append("" if value is None else str(value))
    if values and any(values):
        return json.dumps(values, ensure_ascii=False, separators=(",", ":"))
    content_hash = _read_string(item.get("contentHash"))
    if content_hash:
        return content_hash
...
def _native_node_partial_message(node: CompiledWorkflowNode) -> str:
    ...
    if binding_id == DEDUPE_BINDING_ID:
        return "Duplicate Record Candidates removed"
    if binding_id == MERGE_BINDING_ID:
