...
void main(){
  ...
  float smoothLen = clamp(length(texture2D(velocity, uv).xy) * 2.2, 0.0, 1.0);
  ...
  float gradedT = floor(t * 18.0 + 0.5) / 18.0;
  float paletteT = mix(t, gradedT, 0.34);
  vec3 fluidColor = texture2D(palette, vec2(paletteT, 0.5)).rgb;
  vec3 col        = mix(bgColor.rgb, fluidColor, t);

  float surface = smoothstep(0.06, 0.22, smoothLen) * (1.0 - smoothstep(0.34, 0.72, smoothLen));
  vec3 surfaceColor = texture2D(palette, vec2(0.88, 0.5)).rgb;
  col += surfaceColor * surface * uSurfaceStrength;
...
export interface PixelLiquidBgProps extends React.ComponentProps<"div"> {
  ...
  mouseForce?: number;
  cursorSize?: number;
  /** velocity diffusion; 0 keeps the sharper default flow */
  viscosity?: number;
  /** continuous rim sampled from the same velocity field as the pixel fluid */
...
export function PixelLiquidBg({
  ...
  useEffect(() => {
    ...
    const sim = new FluidSim(gl, mouse, {
      ...
      BFECC: false,
      isBounce: false,
      isViscous: viscosity > 0,
      viscous: viscosity,
      iterations_viscous: 12,
    ...
    const outputUniforms: Uniforms = {
      ...
      uSurfaceStrength: { value: surfaceStrength },
