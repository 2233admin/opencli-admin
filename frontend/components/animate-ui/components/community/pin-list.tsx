...
function PinList({
  ...
  const toggleStatus = (id: React.Key) => {
          ...
          {pinned.length > 0 && (
            ...
            >
              {pinned.map((item) => (
                <motion.button
                  key={item.id}
                  layoutId={`item-${String(item.id)}`}
                  onClick={() => toggleStatus(item.id)}
                  type="button"
                  aria-pressed="true"
                  transition={transition}
                  className="flex w-full items-center justify-between gap-5 rounded-2xl bg-neutral-200 p-2 text-left dark:bg-neutral-800"
                >
            ...
            >
              {unpinned.map((item) => (
                <motion.button
                  ...
                  aria-pressed="false"
                  transition={transition}
                  className="group flex w-full items-center justify-between gap-5 rounded-2xl bg-neutral-200 p-2 text-left dark:bg-neutral-800"
                >
                  <div className="flex items-center gap-2">
                    <div className="rounded-lg bg-background p-2">
                      <item.icon className="size-5 text-neutral-500 dark:text-neutral-400" />
