  ...
  async function persistProject(useBlank = false) {
    ...
    try {
      ...
      const result = await bootstrapProject.mutateAsync({
        workspaceId,
        data: {
          project: { name: finalName, slug: `${studioSlug(finalName)}-${Date.now().toString(36)}`, description: userRequirements.join('；') || '从空白画布创建', app_type: useBlank ? 'workflow' : 'agent' },
          workflow: { name: finalName, description: userRequirements.join('；') || '空白工作流', graph },
        },
      })
      toast.success(spec && !useBlank ? '项目草稿已保存，可以继续验证并发布' : '空白项目已创建')
      router.push(`/studio/workflow?workspace=${workspaceId}&project=${result.project.id}&workflow=${result.primary_workflow.id}${useBlank ? '&guide=blank' : ''}`)
    } catch (reason) { toast.error(reason instanceof Error ? reason.message : '创建失败') }
  }

  return (
    <PageContainer title="与 Agent 创建监测项目" eyebrow="Studio · Agent builder" description="说清楚关注什么、多久检查一次、简报发到哪里。Agent 会整理成可验证、可发布的 Workflow Draft。" className="max-w-none" actions={<Button size="icon" variant="ghost" nativeButton={false} render={<Link href="/studio" />} aria-label="关闭"><X className="size-4" /></Button>}>
